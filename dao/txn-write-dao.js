...
reopenShift: async (closingId, locationCode, userId) => {
    return db.sequelize.transaction(async (t) => {
        ...
        for (const r of claimedReceipts) {
            // Precise match via source_table/source_id (see
            // add-source-tracking-to-cashflow-transaction.sql /
            // add-source-tracking-to-generate-cashflow.sql). Also run the
            // description-based fallback unconditionally - both are no-ops when
            // nothing matches, and it's needed for any t_cashflow_transaction row
            // generated by generate_cashflow before that migration landed (won't
            // have source_id populated until its cashflow is next regenerated).
            await db.sequelize.query(
                `DELETE FROM t_cashflow_transaction
                 WHERE cashflow_id = :cashflowId AND source_table = 't_receipts' AND source_id = :receiptId
                 LIMIT 1`,
                { replacements: { cashflowId: r.pending_cashflow_id, receiptId: r.treceipt_id }, transaction: t }
            );
            await db.sequelize.query(
                `DELETE FROM t_cashflow_transaction
