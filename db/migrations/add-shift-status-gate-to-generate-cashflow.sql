-- ============================================================
-- generate_cashflow: don't claim a Cash Receipt whose originating
-- shift isn't CLOSED yet
--
-- Root cause of the "receipt stays claimed after shift reopen" saga:
-- cur_cash_receipts had no awareness of the originating shift's
-- status at all - it would happily claim a Collections-tab receipt
-- entered on a shift that's still DRAFT (never closed). Once
-- claimed, that receipt is indistinguishable from one entered
-- against a genuinely closed shift, and the earlier reopenShift fix
-- (PR #849/#850) only fires on an actual CLOSED->DRAFT transition -
-- a receipt claimed while its shift was *never* closed has no such
-- transition to trigger cleanup.
--
-- Fix: a receipt is only claimable if it's not tied to any shift
-- (closing_id IS NULL - e.g. entered via the standalone admin
-- Credit Receipts page) OR its shift's closing_status = 'CLOSED'.
-- Added identically to both the cur_cash_receipts cursor and the
-- UPDATE t_receipts that re-claims matching rows after the loop -
-- they must stay consistent, or a receipt could get claimed in
-- t_receipts without a matching t_cashflow_transaction row ever
-- being inserted for it.
--
-- Builds on add-source-tracking-to-generate-cashflow.sql (must be
-- applied first). Full re-declaration. Safe to re-run: uses
