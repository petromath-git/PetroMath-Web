-- ============================================================
-- generate_cashflow: don't claim an Employee Advance/Payment/
-- Recovery entry whose originating shift isn't CLOSED yet
--
-- Same bug class fixed for Cash Receipts in
-- add-shift-status-gate-to-generate-cashflow.sql: cur_salary_payout
-- and cur_salary_recovery had no awareness of the originating
-- shift's status - now that t_employee_ledger gains a closing_id
-- (see add-closing-id-to-employee-ledger.sql) for entries made from
-- the shift-closing "Employee Advance" tab, an entry made on a
-- still-DRAFT shift could otherwise get claimed by generate_cashflow
-- before the shift is ever closed, with no reopen-triggered release
-- path to clean it up afterward.
--
-- Fix: an employee-ledger row is only claimable if it's not tied to
-- any shift (closing_id IS NULL - e.g. entered via the admin
-- Employee ledger modal) OR its shift's closing_status = 'CLOSED'.
-- Added identically to cur_salary_payout, cur_salary_recovery, and
-- the UPDATE t_employee_ledger that re-claims matching rows after
-- the loops - they must stay consistent, or a row could get claimed
-- without a matching t_cashflow_transaction row ever being inserted
-- for it.
--
-- Builds on add-shift-status-gate-to-generate-cashflow.sql (must be
-- applied first - this is a full re-declaration of the same
-- procedure, verified against the live copy via SHOW CREATE
-- PROCEDURE on 2026-08-27, with no other changes made).
